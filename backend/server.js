...
function generateSalesInvoicePDF({ invoiceNo, date, customer, items, amount }) {
  return new Promise((resolve, reject) => {
    ...
    doc.moveDown();
    doc.font("Helvetica-Bold")
       .text(`Total Amount: ₹ ${amount.toFixed(2)}`, { align: "right" });
...
db.serialize(() => {
  ...
  db.get(
    `SELECT COUNT(*) AS cnt FROM ledger_group_master`,
    [],
    (err, row) => {
      if (err) {
      ...
      console.log("Ledger group count at startup:", row.cnt);

      if (row.cnt === 0) {
        db.run(
          `
          INSERT INTO ledger_group_master (group_name, nature) VALUES
          ...
          err => {
            if (err) {
              ...
              console.log("Ledger groups inserted successfully");
            }
          }
        );
        // Queued right after the groups insert (same connection, same tick)
        // so ledger_master's FK reference always finds its groups.
        seedSystemLedgers();
      } else {
        seedSystemLedgers();
      }
...
app.post("/sales/save", async (req, res) => {
  const { date, customer, invoiceNo, items } = req.body;
  ...
  await new Promise((resolve, reject) => {
    ...
    /* 1️⃣ ACCOUNTING ENTRY */
    await saveJournalInternal({
      date,
      narration: `Sales Invoice ${invoiceNo}`,
      entries: [
        { particulars: customer, debit: totalAmount, credit: 0 },
...
app.post("/purchase/save", async (req, res) => {
  const { date, supplier, invoiceNo, items } = req.body;
